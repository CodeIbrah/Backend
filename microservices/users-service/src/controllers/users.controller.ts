...
export async function getUserHandler(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (!user) {
      res.status(404).json(errorResponse('USER_NOT_FOUND', `User with id ${req.params.id} not found`));
      return;
    }
...
export async function updateUserHandler(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (!idValidation.success) {
      res.status(400).json(
        errorResponse('VALIDATION_ERROR', idValidation.error.issues.map((e: z.ZodIssue) => e.message).join(', '))
      );
      return;
    }
