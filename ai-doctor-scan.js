...
function getAllFiles(
  dir,
  patterns,
  excludeDirs = ['node_modules', '.git', 'dist', 'build', '.turbo'],
) {
  const results = [];
...
function scanFile(filePath) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      /(?:api[_-]?key|secret[_-]?key|access[_-]?token|auth[_-]?token|private[_-]?key)\s*[:=]\s*['"][^'"]{8,}['"]/i.test(
        line,
      ) &&
      !line.includes('process.env') &&
      !line.includes('configService')
    ...
    if (
      /(?:exec|execSync|spawn|spawnSync|execFile)\s*\(/.test(line) &&
    ...
    if (
      /res\.(send|end|write)\s*\(/.test(line) &&
      (line.includes('req.') || line.includes('body') || line.includes('query'))
    ) {
    ...
    if (/\w+\.findUnique\s*\(/.test(line) || /\w+\.findOne\s*\(/.test(line)) {
      ...
      if (
        ...
        !nextLines.includes('if (') === false &&
        !nextLines.includes('?.')
      ) {
        // Only flag if there's no immediate null check
        const hasNullCheck = lines
          .slice(i + 1, i + 8)
          .some((l) => /if\s*\(\s*!?\w+\s*\)/.test(l) || /\?\./.test(l));
        if (!hasNullCheck) {
          addFinding(
            filePath,
            lineNum,
    ...
    if (/for\s*\(/.test(line) || /\.forEach\s*\(/.test(line) || /\.map\s*\(/.test(line)) {
      ...
      if (
        nextLines.includes('.find') ||
        nextLines.includes('.create') ||
        nextLines.includes('.update') ||
        nextLines.includes('.delete') ||
    ...
    if (/\.push\s*\(/.test(line) && !line.includes('MAX') && !line.includes('limit')) {
      ...
      if (
        !nearbyLines.includes('.length') &&
        !nearbyLines.includes('slice') &&
        !nearbyLines.includes('splice')
      ) {
    ...
    if (
      /cors\s*\(\s*\{\s*origin\s*:\s*['"]\*['"]/i.test(line) ||
    ...
    if (
      isNestJS &&
      (line.includes('@Post(') || line.includes('@Patch(') || line.includes('@Delete('))
    ) {
      if (
        line.includes('auth') ||
        line.includes('login') ||
    ...
    if (/new\s+Map\s*\(\s*\)/.test(line) || /new\s+Set\s*\(\s*\)/.test(line)) {
      ...
      if (varName) {
        ...
        if (
          !restOfFile.includes(`${name}.size`) &&
          !restOfFile.includes(`${name}.delete`) &&
          !restOfFile.includes('MAX') &&
          !restOfFile.includes('limit')
        ) {
          addFinding(
            filePath,
            lineNum,
    ...
    if (
      /res\.redirect\s*\(/.test(line) &&
      (line.includes('req.') || line.includes('query') || line.includes('body'))
    ) {
      addFinding(
        filePath,
        lineNum,
    ...
    if (
      ...
      i === lines.findIndex((l) => l.includes('app.use('))
    ) {
  ...
  if (
    fileContent.includes('PrismaClient') &&
    !fileContent.includes('datasource') &&
    isInfra === false
  ) {
...
function findFunctionStart(lines, currentLine) {
  for (let i = currentLine; i >= 0; i--) {
    if (
      /^(async\s+)?function\s+|^(export\s+)?(const|let|var)\s+\w+\s*=\s*(async\s+)?\(/.test(
        lines[i].trim(),
      )
    ) {
      return i;
...
function scanSpecificPatterns(filePath) {
  ...
  if (relPath.includes('auth.middleware')) {
    if (
      ...
      !fileContent.includes('jwt') &&
      !fileContent.includes('jsonwebtoken')
    ) {
      addFinding(
        filePath,
        1,
  ...
  if (relPath.includes('jwt.strategy')) {
    if (fileContent.includes('payload.email') && !fileContent.includes('email')) {
      ...
      const authServiceContent = fs.readFileSync(
        path.join(PROJECT_ROOT, 'main/src/auth/auth.service.ts'),
        'utf-8',
      );
  ...
  if (relPath.includes('validation.ts') && relPath.includes('shared')) {
    if (
      fileContent.includes('sanitizeInput') &&
      !fileContent.includes('script') &&
      !fileContent.includes('onerror')
    ) {
      addFinding(
        filePath,
        1,
...
const reportPath = path.join(
  reportDir,
  `ai-doctor-scan-${new Date().toISOString().replace(/[:.]/g, '-')}.json`,
);
